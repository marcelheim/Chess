...
public class King extends Piece{
    ...
    public void createMoves(Cell[][] board) {
        ...
        if(newX < 8 && newY < 8){
            ...
            else getMoveList().add(new Move(MoveType.Capture, new Position(newX, newY)));
        }
        newX = x + 1;
        if(newX < 8){
            if(board[newX][y].getPiece() == null) getMoveList().add(new Move(MoveType.Move, new Position(newX, y)));
            else getMoveList().add(new Move(MoveType.Capture, new Position(newX, y)));
        }
        newX = x + 1;
        newY = y - 1;
        if(newX < 8 && newY >= 0){
        ...
        if(newY >= 0){
            ...
            else getMoveList().add(new Move(MoveType.Capture, new Position(x, newY)));
        }
        newX = x - 1;
        newY = y - 1;
        if(newX >=0 && newY >= 0){
